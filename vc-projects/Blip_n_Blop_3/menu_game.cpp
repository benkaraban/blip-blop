...
void MenuGame::updateName()
{
	...
	switch (current_menu) {
		case MENU_MAIN:
			strcpy(menu_txt[0], txt_data[TXT_RESUME].c_str());
			strcpy(menu_txt[1], txt_data[TXT_OPTIONS].c_str());
			strcpy(menu_txt[2], txt_data[TXT_EXIT_GAME].c_str());
			break;

		case MENU_OPTS:

			if (vSyncOn)
				sprintf(menu_txt[0], "%s %s", txt_data[TXT_VSYNC].c_str(), txt_data[TXT_ON].c_str());
			else
				sprintf(menu_txt[0], "%s %s", txt_data[TXT_VSYNC].c_str(), txt_data[TXT_OFF].c_str());
...
//		strcpy( menu_txt[1], txt_data[TXT_LANGUAGE]);
			...
			break;
                ...
                case MENU_KEY1: {
                    auto keys = {
                        std::make_tuple(0, TXT_UP, ALIAS_P1_UP),
                    ...
                    for (auto& k : keys) {
                        if (redefine == std::get<0>(k)) {
                            ...
                            strcpy(menu_txt[std::get<0>(k)], txt.c_str());
                        } else {
                            std::string txt =
                                txt_data[std::get<1>(k)] + " = " +
                ...
                case MENU_KEY2: {
                    auto keys = {
                        std::make_tuple(0, TXT_UP, ALIAS_P2_UP),
